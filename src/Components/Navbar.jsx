        ...
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-sticky">
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-[99px] bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-black">
              <li>
                <a href="/" className="block py-2 pl-3 pr-4 text-gray-300 rounded hover:text-gold hover:bg-transparent md:p-0">
                  Home
                </a>
              </li>
              <li className="relative group">
                <button onClick={toggleDropdown} className="flex items-center focus:outline-none text-gray-300 hover:text-gold ml-3">
                  <span className="mr-1">Service</span>
                  <span className={`ml-1 ${isDropdownOpen ? 'rotate-180' : 'rotate-0'} transition-transform duration-300`}>&#9662;</span>
                </button>
                {isDropdownOpen && (
